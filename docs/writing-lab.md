...
`writing-lab-audio.js` serves the 13 checked-in MP3s from `content/writing-audio` before considering on-demand narration. Playback works with an empty Railway cache and without a narration API key or quota. The manifest records transcript and audio SHA-256 hashes, byte counts, duration and voice. `npm ci` validates every recording, so missing, truncated or outdated audio blocks deployment. File responses support HTTP byte ranges for browser loading and resuming; errors are not cached. Versioned audio URLs and the client asset URL avoid stale responses. Bundled media requests do not use the former 30-request narration quota, which could interrupt classrooms sharing an IP.
