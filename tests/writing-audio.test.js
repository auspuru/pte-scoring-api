...
test('User SST masters use OpenAI quality-optimized HD TTS and never robotic local speech', async () => {
  ...
  const nextBlock=source.indexOf('if(process.env.OPENAI_API_KEY)',neuralStart);
  assert(nextBlock>neuralStart,'OpenAI HD path must be the runtime-neural provider');
  const neuralBlock=source.slice(neuralStart,source.indexOf('\n    }\n    if(process.env.OPENAI_API_KEY)',nextBlock));
