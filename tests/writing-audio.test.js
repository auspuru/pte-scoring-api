...
test('Neural SST prefers verified native-HD Edge audio and never falls back to robotic local speech', async () => {
  ...
  assert.match(source,/Edge native HD narration selected/);
  ...
  const edgeCall=source.indexOf('createEdgeNarration(input)',neuralStart);
  const openAiCall=source.indexOf('if(process.env.OPENAI_API_KEY)',neuralStart);
  assert(edgeCall>neuralStart && edgeCall<openAiCall,'native HD Edge must be attempted before OpenAI');
  const neuralBlock=source.slice(neuralStart,source.indexOf("if(process.env.OPENAI_API_KEY)",openAiCall+1));
