(function () {
  ...
  function renderResults() {
    ...
    root.innerHTML='<section class="results"><div class="results-header"><div><p class="eyebrow" style="color:#287e8a">Attempt complete</p><h1>'+esc(attempt.title)+'</h1><p class="muted">'+new Date(attempt.startedAt).toLocaleString()+' · Saved to '+esc(username)+'</p></div><div class="results-actions"><button class="secondary" data-tab="history">My attempts</button>'+retry+'</div></div><div class="score-banner"><div class="score-total">'+(scored?summary.total:'—')+'<small> / '+(summary.maximum||'—')+'</small></div><div><h2>Native practice result</h2><p>'+(scored?'Task marks are shown first. The '+summary.score90+'/90 figure is a secondary practice estimate, not an official PTE score.':'Your answers are submitted. Preparing your assessment…')+'</p>'+(scored?'<p class="muted">Practice estimate: '+summary.score90+'/90</p>':'')+'</div></div><div class="task-scores">'+summary.byType.map(g=>'<div class="task-score"><span>'+esc(g.label)+'</span><strong>'+(g.marked===g.count?g.total:'—')+'<small> / '+g.maximum+'</small></strong><small>'+g.count+' question'+(g.count===1?'':'s')+(g.score90==null?' · Feedback pending':' · estimate '+g.score90+'/90')+'</small></div>').join('')+'</div><div id="scoring-status" class="scoring-status"></div>'+attempt.questions.map((q,i)=>reviewCard(q,i)).join('')+practiceNavigation()+'</section>';
    root.focus();
