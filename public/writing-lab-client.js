  ...
  async function scoreRemaining() {
    ...
    try {
      ...
      if(failed && attempt?.id===id) {
        const status=document.getElementById('scoring-status');
        if(status) status.innerHTML='Some assessments are still pending. Your completed assessments and answers are saved. <button class="secondary" id="retry-scores">Retry assessment</button>';
