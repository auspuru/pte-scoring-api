...
(function (root, factory) {
  ...
  function explanation(q,a) {
    ...
    if (q.type === 'swt') {
      ...
      return `<section class="reading-explanation"><h3>${p.pending?'SWT grade pending':p.earned+'/'+p.possible+' SWT points'}</h3>${p.pending?`<p>${escape(assessment?.message || 'Your response is saved. The existing SWT grader will assess it.')}</p><button class="portal-button" data-action="retry-swt" ${assessment?.status==='working'?'disabled':''}>${assessment?.status==='working'?'Assessing your summary…':'Retry SWT grading'}</button>`:traits?`<p>Content ${traits.content} · Form ${traits.form} · Grammar ${traits.grammar} · Vocabulary ${traits.vocabulary}</p>`:'<p>No summary was submitted.</p>'}${q.sampleResponse?`<details><summary>Example summary</summary><p>${escape(q.sampleResponse)}</p></details>`:''}</section>`;
    }
  ...
  async function gradeSwt(q) {
    ...
    s.assessments[q.uid]={status:'working',message:'Assessing your summary with the existing SWT grader…'};
    ...
    try {
      ...
      s.assessments[q.uid]={status:pending?'error':'complete',result,message:pending?'The grader returned a provisional result. Retry to obtain a confirmed SWT grade.':'SWT assessment complete.'};
    } catch (error) {
      if (ticket!==generation || gradingOwner!==identity() || token!==authToken()) return;
