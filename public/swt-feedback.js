...
(function (root, factory) {
  ...
  const compact = (value, max = Infinity) => { const text = clean(value).replace(/\s+/g, ' '); return Number.isFinite(max) && text.length > max ? text.slice(0, Math.max(0, max - 1)).trimEnd() + '…' : text; };
  function build(data) {
