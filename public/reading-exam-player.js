(function (root, factory) {
  ...
  function render({ session:s, question:q, content, audio, timer, saved, notice }) {
    ...
    return `<div class="reading-exam-player" data-exam-player>
      <header class="reading-exam-header"><div class="reading-exam-brand"><img src="assets/ipt-brisbane-logo.png" alt="IPT Brisbane"><div><strong>IPT Brisbane</strong><span>${escape(s.name)}</span></div></div><div class="reading-exam-clock"><span>Time remaining</span><strong data-timer>${escape(timer)}</strong><span>Question ${s.index+1} of ${s.questions.length}</span></div></header>
      <div class="reading-exam-stage"><div class="reading-exam-section"><span>${section}</span><button type="button" data-action="exam-exit">Exit test</button></div><article class="reading-exam-question" aria-labelledby="readingExamTitle"><h2 id="readingExamTitle" data-exam-title tabindex="-1">${titles[q.type]}</h2><p class="reading-exam-instructions">${escape(q.instructions)}</p>${audio||''}<fieldset><legend class="sr-only">Your answer</legend>${content}</fieldset></article></div>
