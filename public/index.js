    ...
    if (isFocusArea) {
      ...
      const colA = leftFiltered.length === 0
        ...
        : (leftFellBack ? fallbackHint : '') + leftFiltered.map((idea) => {
            ...
            return `
              <div class="idea-item ${isSelected ? 'selected' : ''}" onclick="toggleIdeaText('main_support', '${escapeHtml(idea.text)}')" style="cursor:pointer; padding:8px 10px; margin-bottom:6px; border:1px solid ${isSelected ? 'var(--accent)' : 'var(--line-soft)'}; border-radius:6px; display:flex; align-items:center; gap:8px; background:${isSelect�<o+^����םize:10px; font-weight:600; cursor:pointer; padding:2px 4px;">
